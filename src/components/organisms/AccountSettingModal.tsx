...
const AccountSettingModal: React.FC<TaskSettingsModalProps> = ({
  isOpen,
  onClose,
  taskSettings
}) => {
  ...
  const fetchAccountSettings = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.vipadtg.com/api/v1/forwarder/account/settings/${taskSettings.accountId}`,
        {
          method: 'GET',
          headers: {
  ...
  const fetchTaskDetails = async () => {
    ...
    try {
      const response = await fetch(
        `https://api.vipadtg.com/api/v1/forwarder/task/${taskSettings.selectedTaskId}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleAccountSettingsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setAccountSettings((prev) => ({
      ...prev,
      [name]:
        name.includes('interval') || name.includes('second') || name.includes('sleep')
          ? parseInt(value) || 0
          : value
    }));
  };
  ...
  const handleMessageGroupChange = (
    index: number,
    field: 'message_link' | 'target_groups',
    value: string | string[]
  ) => {
    setTaskData((prev) => {
